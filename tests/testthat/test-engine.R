test_that("Engine calculates stats correctly (Math check)", {
  ...
  mat_y <- Matrix::Matrix(matrix(c(0,0,1,1,0), nrow=1), sparse=TRUE)
  ...
  expect_equal(res$size_y, 2)

  # Derived stats
  # Jaccard = Int / Union = 1 / 4 = 0.25
  expect_equal(res$jaccard, 0.25)

  # Overlap = Int / min(size_x, size_y) = 1 / 2
  expect_equal(res$overlap, 1/2)

  # P-value Check
  # phyper(q, m, n, k) -> q=int-1, m=size_x, n=total-size_x, k=size_y
  expected_p <- phyper(0, 3, 2, 2, lower.tail = FALSE)
