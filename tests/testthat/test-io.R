...
test_that("import_coreact_tsv handles index-based meta columns", {
  tf <- tempfile(fileext = ".tsv")
  df <- data.frame(ID=1:5, Val=1:5)
  write.table(df, tf, sep="\t", row.names=FALSE)

  obj <- import_coreact_tsv(tf, meta_cols = 1)
