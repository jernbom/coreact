...
#' @param prev_x_chunk Vector of prevalence for rows in X chunk.
#' @param prev_y Vector of prevalence for all rows in Y.
#' @param n_samples Integer, total number of samples.
#' @param config List of name-value pairs of filter thresholds. Current valid options are `min_intersection`, `min_jaccard`, and `min_overlap`.
#'
...
worker_chunk_calc <- function(idx_x_chunk,
  ...
  # BLAS optimized dense matrix mult.
  ...
  # Only compute secondary matrices if used for FILTERING.
